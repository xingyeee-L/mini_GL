...
el('chat-import').addEventListener('submit',async e=>{e.preventDefault();const status=el('chat-status');try{status.className='muted';status.textContent='正在只读校验、切片并原子导入…';const path=new FormData(e.target).get('path');const out=await api('/api/chat-import',{method:'POST',body:JSON.stringify({path})});status.textContent=`导入成功：${out.documents} 个会话片段 · 新增 ${out.created} · 更新 ${out.updated} · 未变化 ${out.unchanged}`;await load(false);el('sources').value=out.source_id;await detail()}catch(x){status.textContent='导入失败：'+x.message;status.className='error'}});
        ...
        try:
            ...
            with SQLiteStore(self.server.db_path) as store:
                ...
                if self.path == "/api/register":
                    ...
                    self._json(ChatImportService(store).import_file(Path(str(body["path"]))))
